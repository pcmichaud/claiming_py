...
class modelbase:
	...
	def init_data(self,s_df, scenarios):
		...
		for j in scenarios:
			for t in self.ages:
				keep.append('gaindelay_'+str(t)+'_'+str(j)+"_lt")
			df = df.loc[~df['gaindelay_'+str(60)+'_'+str(j)+"_lt"].isna(),:]
		self.gains_labels = ['gaindelay_'+str(a)+'_'+str(j)+"_lt" for j in scenarios for a in self.ages]
		for j in scenarios:
			df['nra_'+str(j)] = 65
			if j==7:
